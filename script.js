(() => {
  ...
  const STORAGE_KEY = "dollMakerEndingData";
  ...
  function persistEndingData() {
    ...
    try {
      ...
      console.warn(
        "Could not save ending data to sessionStorage:",
        error
      );
    }
    ...
    try {
      ...
      console.warn(
        "Could not save ending data to localStorage:",
        error
      );
    }
  }

  function restoreEndingData() {
    let savedData = null;
    ...
    try {
      ...
      console.warn(
        "Could not read saved ending data from sessionStorage:",
        error
      );
    }
    ...
    if (!savedData) {
      try {
        ...
        console.warn(
          "Could not read saved ending data from localStorage:",
          error
        );
      }
    }
  ...
  function applyCurrentStateImmediately() {
    if (!state.ending) {
      return;
    }
  ...
  function openCredits() {
    if (!state.ending) {
      console.error(
        "Credits could not open because no ending is loaded."
      );
    ...
    /*
     * Never return to the fixed empty iframe URL. Credits receives a
     * complete ending-screen URL containing the ending and letter count.
     * This works even if Portals reloads the iframe or clears page state.
     */
    const returnUrl =
      new URL(
  ...
  /*
   * Priority:
   * 1. URL parameters, for browser testing.
   * 2. Saved session data, for Credits -> Back.
   * 3. A Portals iframe message arriving afterwards.
   */
  readQueryParameters();
